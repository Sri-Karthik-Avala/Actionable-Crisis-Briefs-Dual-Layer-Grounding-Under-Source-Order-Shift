...
train = pd.read_csv(public_dir / "train.csv").iloc[::8].reset_index(drop=True)
test = pd.read_csv(public_dir / "test.csv").iloc[:60].reset_index(drop=True)
...
class Tagger(nn.Module):
    def __init__(self, backbone, head):
        ...
        if head == "bio":
            ...
            self.arg_head = SpanHead(hidden, len(ARGUMENT_ROLES))
            self.ent_head = SpanHead(hidden, len(ENTITY_TYPES))

    def forward(self, ids, att, first):
        h = self.encoder(input_ids=ids, attention_mask=att).last_hidden_state
        g = self.dropout(h.gather(1, first[:, :, None].expand(-1, -1, h.size(-1))))
...
def member_loss(model, la, le, lengths, ya, ye, sa, se):
    W = la.size(1)
    if model.head == "bio":
    ...
    return F.cross_entropy(la[sm], sa[sm]) + F.cross_entropy(le[sm], se[sm])
...
def train_member(cfg, rows, pad_id, predict_sets):
    ...
    for epoch in range(cfg["epochs"]):
        ...
        for idx in batches:
            ...
            with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=use_amp):
                la, le = model(ids, att, first)
            loss = member_loss(model, la, le, lengths, ya, ye, sa, se)
...
def count_above(probs_list, label_index, threshold):
    total = 0
    for probs in probs_list:
        lp = probs[:, :, 1:]
        best = lp.argmax(-1)
        best_p = np.take_along_axis(lp, best[:, :, None], -1)[:, :, 0]
        n = probs.shape[0]
        for s in range(n):
            L = min(MAX_SPAN, n - s)
            total += int(np.sum((best[s, :L] == label_index) & (best_p[s, :L] > threshold)))
    return total


def prior_thresholds(probs_list, labels, rate, n_docs):
    out = []
    for j, lab in enumerate(labels):
        target = rate[lab] * n_docs
        lo, hi = 0.05, 0.9
        for _ in range(11):
            mid = 0.5 * (lo + hi)
            if count_above(probs_list, j, mid) > target:
                lo = mid
            else:
                hi = mid
        out.append(min(max(0.5 * (lo + hi), THRESHOLD_FLOOR), THRESHOLD_CEIL))
    return np.array(out)
...
rate_arg = {r: 0.0 for r in ARGUMENT_ROLES}
rate_ent = {t: 0.0 for t in ENTITY_TYPES}
for i in calib_fit["id"]:
    t = target_of[i]
    for a in t["arguments"]:
        rate_arg[a["role"]] += 1.0 / len(calib_fit)
    for e in t["entities"]:
        rate_ent[e["type"]] += 1.0 / len(calib_fit)
...
arg_thresholds = prior_thresholds(hold_a, ARGUMENT_ROLES, rate_arg, len(calib_hold))
ent_thresholds = prior_thresholds(hold_e, ENTITY_TYPES, rate_ent, len(calib_hold))
print("global threshold", round(global_threshold, 4), flush=True)
print("argument thresholds", {r: round(float(t), 3) for r, t in zip(ARGUMENT_ROLES, arg_thresholds)}, flush=True)
print("entity thresholds", {t: round(float(v), 3) for t, v in zip(ENTITY_TYPES, ent_thresholds)}, flush=True)
